...
function showRecentSearches() {
    ...
    if (searches.length > 0 && suggestionsList) {
        const html = searches.map((item, index) => {
            const safeItem = item.replace(/'/g, "\\'");
            return `<li class="recent-item" onclick="selectSuggestion('${safeItem}')"> <div class="recent-content"> <span> ${item} </span> </div> <span class="remove-btn" onclick="removeRecentSearch(event, ${index})">X</span> </li>
        `;
        }).join('');
...
function displaySuggestions(list) {
    const html = list.map((item) => { return `<li onclick="selectSuggestion('${item.replace(/'/g, "\\'")}')">${item}</li>`; }).join('');
    const suggestionsList = document.getElementById('suggestions-list');
    if (suggestionsList) {
